...
impl<A: Unpin, B: Unpin, C: Unpin> Unpin for Select<A, B, C> {}

pub fn select<A, B, C>(future1: A, future2: B, future3: C) -> Select<A, B, C>
where
    A: Future + Unpin,
    B: Future + Unpin,
    C: Future + Unpin,
{
    assert_future::<Option<C::Output>, _>(Select {
        inner: Some((future1, future2, future3)),
    })
}

impl<A, B, C> Future for Select<A, B, C>
where
    A: Future + Unpin,
    B: Future + Unpin,
    C: Future + Unpin,
{
    ...
    // Returns if any of the futures exits, if the last future (future3: C) exits the function returns Some(C::Output) 
